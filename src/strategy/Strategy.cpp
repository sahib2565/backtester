...
Strategy::Strategy(std::size_t rsiPeriod, std::size_t slowPeriod,
                   std::size_t fastPeriod)
    : inTrade(false), emaFast(EMA(fastPeriod)), emaSlow(EMA(slowPeriod)),
      rsi(RSI(rsiPeriod)), lastRSI(0), hasLastRsi(0) {
...
int Strategy::update(double close) {
  ...
  if (!rsi.ready()) {
    ...
    if (!inTrade) {
      if (emaFast.value() > emaSlow.value() && lastRSI < 30 &&
          rsi.value() >= 30) {
        inTrade = true;
        return 1;
      }
